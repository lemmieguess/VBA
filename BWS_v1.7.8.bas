...
Public  Const BWS_VERSION   As String = "v1.7.8"
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ...
    If Not bodyCC Is Nothing Then
                ...
                If LenB(txt) > 0 Then
                    ...
                    If InStr(1, txt, "Sincerely", vbTextCompare) > 0 Or _
                       ...
                       InStr(1, txt, "Regards", vbTextCompare) > 0 Then
                        ' Only delete if NOT inside signature wrapper content control
                        isInSigWrapper = False
                        For Each cc In doc.ContentControls
                            If InStr(1, cc.Title, "signature", vbTextCompare) > 0 Or _
                               InStr(1, cc.Tag, "signature", vbTextCompare) > 0 Then
                                ' Check if paragraph is inside this signature wrapper
                                If para.Range.Start >= cc.Range.Start And _
                                   para.Range.End <= cc.Range.End Then
                                    isInSigWrapper = True
                                    Exit For
                                End If
                            End If
                        ...
                        ' Only add to delete list if NOT in signature wrapper
                        If Not isInSigWrapper Then
        ...
        For i = parasToDelete.Count To 1 Step -1
            ...
            ' BUT don't extend beyond body content boundary
            ...
            ' Delete the entire range (up to 6 paragraphs) at once
            deleteRange.Delete
            On Error GoTo 0
        Next i
    End If
...
Public Sub BWS_About()
    ...
    ' Part 1: Header and recent versions (v1.7.8, v1.7.7, v1.7.6)
    msg = "================ BWS v1.7.8 ==================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.8:" & vbCrLf _
        & "• Fixed signature image deletion" & vbCrLf _
        & "• Fixed bullet vertical alignment" & vbCrLf _
        & vbCrLf _
        & "From v1.7.7:" & vbCrLf _
        & "• Fixed Paragraph.Index error" & vbCrLf _
        & vbCrLf _
        & "From v1.7.6:" & vbCrLf _
        & "• Fixed Dim statements inside loop" & vbCrLf
