...
' ============================================================================
' Bridgewater Studio - BWS v1.6.5 - Bullet Indent, Table Order, Signature Fix
' Based on: v1.6.4 with three critical formatting fixes
'
' v1.6.5 FIXES:
...
' - Fixed signature block: Now ONLY moves content, does NOT apply formatting
'   Template formatting is preserved, minimal whitespace
'
...
Public  Const BWS_VERSION   As String = "v1.6.5"
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.6.5 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ...
    Dim sigEnd As Paragraph
    Dim sigRange As Range
    ...
    ' If found, identify signature block range (greeting + next 3 lines)
    If Not sigStart Is Nothing Then
        ...
        ' If we found the signature block, move it to end of document
        ' Cut from current position and paste at end (preserves all formatting)
        If Not sigEnd Is Nothing Then
            Set sigRange = doc.Range(sigStart.Range.Start, sigEnd.Range.End)
            sigRange.Cut
            ...
            ' Remove excess whitespace before signature block
            endRange.Collapse wdCollapseStart
            endRange.MoveStart wdParagraph, -3
            endRange.Select
            Selection.Delete
        End If
    End If
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.6.5 toolbar installed!" & vbCrLf & vbCrLf & _
