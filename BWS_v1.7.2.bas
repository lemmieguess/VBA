...
' ============================================================================
' Bridgewater Studio - BWS v1.7.2 - Critical Import Fixes
' Based on: v1.7.1 with complete rewrite of signature and whitespace handling
'
' v1.7.2 BUGFIXES:
' - Fixed top margin: Changed from -0.062" to 0.5" (user requirement)
'   Content no longer appears at very top of page
' - Fixed signature duplication: ONLY process template signature wrapper
'   Completely rewrote FormatSignatureBlock to find signature wrapper content control
'   Ignores all "Sincerely" signatures in body content (no processing/spacing)
'   Result: Only ONE signature (template) appears in final document
' - Fixed signature image: IN FRONT of text (msoBringToFront in wrapper)
'   Image in signature block wrapper now properly formatted
' - Fixed whitespace: Aggressive loop-based stripping from body content
'   Removes ALL leading/trailing empty paragraphs until content found
'
' v1.7.1 BUGFIXES (incomplete - see v1.7.2):
...
Public  Const BWS_VERSION   As String = "v1.7.2"
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.2 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub ImportDocument(ByVal sourcePath As String)
    ...
    ' 2. Strip whitespace from body content control
    StripBodyContentWhitespace doc
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ' Process ONLY the signature block wrapper (template signature)
    ' Do NOT process signatures in body content (imported document)
    ' Fix image formatting in signature wrapper to be "in front of text"
    ...
    ' Find signature block content control (NOT body content)
    Set sigCC = Nothing
    For Each cc In doc.ContentControls
        ' Look for signature block wrapper by title/tag
    ...
    ' If signature wrapper found, fix image formatting
...
Private Sub StripBodyContentWhitespace(ByVal doc As Document)
    ' Strip whitespace from INSIDE body content control only
    ' Aggressive: keeps removing empty paragraphs until content found
    Dim bodyCC As ContentControl
    Dim para As Paragraph
    ...
    If bodyCC Is Nothing Then
        ' No body content, use whole document
        StripLeadingWhitespace doc
        StripTrailingWhitespace doc
        Exit Sub
    End If

    ' Strip leading empty paragraphs (up to 50 attempts)
    attempts = 0
    Do While attempts < 50
        found = False
        For Each para In doc.Paragraphs
            ' Check if paragraph is in body content control
    ...
    ' Refresh body content control reference
    Set bodyCC = FindBodyContentControl(doc, False)
    If bodyCC Is Nothing Then Exit Sub

    ' Strip trailing empty paragraphs (up to 50 attempts)
    attempts = 0
    Do While attempts < 50
        found = False
        Dim i As Long
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.7.2 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    msg = "================ BWS v1.7.2 ==================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.2:" & vbCrLf _
        & "• Top margin: 0.5"" (was negative)" & vbCrLf _
        & "• Signatures: Template wrapper ONLY" & vbCrLf _
        & "• Picture: IN FRONT in wrapper" & vbCrLf _
        & "• Whitespace: Aggressive stripping" & vbCrLf _
        & vbCrLf _
        & "From v1.7.0:" & vbCrLf _
        & "• One-click import workflow!" & vbCrLf _
        & "• Import Newest: Creates doc + imports" & vbCrLf _
        & vbCrLf _
        ...
        & "• Table order: Fixed alignment override" & vbCrLf
