...
' ============================================================================
' Bridgewater Studio - BWS v1.7.29 - Fix UserForm Compile Error
' Based on: v1.7.28 with UserForm integration
'
' v1.7.29 BUGFIX:
' - Fixed "Variable not defined" compile error in BWS_Settings()
'   * Issue: Direct reference to BWS_SettingsForm.Show caused compile error
'   * Root cause: VBA checks all code at compile time, even if UserForm doesn't exist
'   * Solution: Use late binding - show the form object directly (frm.Show)
'   * Now compiles successfully whether UserForm exists or not
'   * Fallback to InputBox still works perfectly
'
' v1.7.28 MAJOR IMPROVEMENTS:
' - Professional UserForm settings dialog (manual setup required)
'   * Proper Windows dialog with buttons, labels, checkboxes
'   * Folder browser integration
'   * File picker for template
'   * Much more sophisticated than InputBox
'   * Falls back to InputBox if UserForm not found
'   * See setup instructions at end of file
'
' - Improved toolbar auto-install (SOP):
'   * ALWAYS uninstalls old toolbar before installing new one
'   * Prevents duplicate toolbars
'   * Clean slate every time
'
' - Robust AutoOpen behavior for Normal.dotm:
'   * Checks if toolbars exist on every Word startup
'   * Recreates if missing (persistent toolbar management)
'   * Silent operation - no popups on startup
'
' - Better toolbar organization:
'   * Visual grouping with separators
'   * Core workflow buttons more prominent
'   * Settings/utilities grouped separately
'
' v1.7.27 BUGFIX:
' - Fixed "too many line continuations" VBA compiler error
'   * VBA has 25 line continuation limit per statement
'   * Settings dialog exceeded this in v1.7.26
'   * Split long string concatenations into multiple statements
'   * Now builds message in parts: msg = part1, msg = msg & part2, etc.
'
' v1.7.26 IMPROVEMENTS:
' - Improved settings interface - menu-driven with better UX
'   * Shows all settings in clean numbered list
'   * Allows multiple changes in one session
'   * Input validation on all entries
'   * "0" to exit, no need to cancel multiple dialogs
'   * Much more professional than old version
'
' - Updated toolbar tooltips for manual formatting buttons
'   * "Fix Tables" → "Manually format tables (auto during import)"
'   * "Convert Bullets" → "Manually convert bullets (auto during import)"
'   * "Fix Bullets" → "Manually fix bullet formatting"
'   * "Apply Format" → "Manually apply formatting (auto during import)"
'   * Clarifies these are for manual cleanup, not regular workflow
'
' v1.7.25 BUGFIX:
' - Removed obsolete signature image conversion code
'   * User protected signature wrapper at template level (perfect solution!)
'   * STEP 2 of FormatSignatureBlock was trying to convert inline shapes
'   * This failed with "Method convert to shape of object in line shape failed"
'   * Since signature is protected in template, conversion is unnecessary
'   * Removed entire STEP 2 - signature formatting now handled by template
'   * STEP 1 (removing imported signatures from body) still needed and kept
'
' v1.7.24 BUGFIX:
' - Fixed "Import Pick" button - now creates new document from template first
'   * Previously: tried to import into active document (no BodyContent control)
'   * Now: matches "Import Newest" workflow - creates template doc first
'   * File picker opens in Drafts folder automatically
'   * PickFile() now accepts optional initialFolder parameter
'
' v1.7.23 USER REQUIREMENT:
' - "Set column 3 preferred width to 1.375"
'   * Col3: 1.375" (changed from 1.7")
'   * PreferredWidth = 99 points (1.375 * 72)
'   * Table width: 7.75" (unchanged)
'   * Col1: 1.45" (unchanged)
'   * Col2: 5" (unchanged)
'
' v1.7.22 USER REQUIREMENT:
' - "Preferred table width wasn't set, set it at 7.75""
'   * PreferredWidth = 558 points (7.75 * 72)
'   * Column widths: Col1=1.45", Col2=5", Col3=1.7" (NOW 1.375" IN v1.7.23)
'
' v1.7.21 USER REQUIREMENTS:
' - "Table width: 7.68"" (not 7") - NOW 7.75" IN v1.7.22
'   * PreferredWidth = 553.92 points (7.68 * 72)
'
' - "Column 1: 1.45"" (Tier column)
'   * PreferredWidth = 104.4 points (1.45 * 72)
'
' - "Column 2: 5"" (Experience Focus / Description column)
'   * PreferredWidth = 360 points (5 * 72)
'
' - "Column 3: 1.375"" (Budget column) - CHANGED IN v1.7.23 from 1.7"
'   * PreferredWidth = 99 points (1.375 * 72)
'
' v1.7.20 USER REQUIREMENTS:
' - "Lost the preferred table width of 7""
'   * Set table width to 7" fixed (not 100% of window)
'   * PreferredWidthType = wdPreferredWidthPoints
'   * PreferredWidth = 504 points (7 * 72) (NOW 7.68" IN v1.7.21)
'
' - "Budget column is off the page and too wide"
'   * Budget column should be tight to numbers (NOW FIXED WIDTH IN v1.7.21)
'   * Let currency columns size to content naturally
'
' - "Experience Focus should be 25% wider" (not 30%)
'   * Changed from 1.3x to 1.25x multiplier (NOW FIXED WIDTH IN v1.7.21)
'
' v1.7.19 USER REQUIREMENT (per 2025-11-14_00-36-45.png feedback):
' - "Bullet point wrapping is great, but bullet marker should be indented 0.25""
'   * Keep text wrapping at 0.5" (working perfectly)
'   * Move bullet marker from 0" to 0.25"
'   * LeftIndent = 0.5" (text position - unchanged)
'   * FirstLineIndent = -0.25" (changed from -0.5")
'   * Result: Bullet at 0.25", text at 0.5" and wraps beautifully
'
' v1.7.18 USER REQUIREMENTS (per 2025-11-13_23-19-17.png feedback):
' - "Everything left-justified with zero margin" - vertical baseline at left edge
'   * Regular paragraphs: LeftIndent = 0" (changed from 0.5")
'   * Creates clean vertical alignment down the left edge
'
' - "Bullet points should be standard half inch with hanging indent"
'   * LeftIndent = 0.5" (changed from 0.25")
'   * FirstLineIndent = -0.5" (hanging indent, changed from -0.25")
'   * Result: Bullet at 0", text at 0.5" and aligns vertically (CHANGED IN v1.7.19)
'
' - "Description column (middle) should be 30% bigger"
'   * Table columns auto-sized, but middle column gets preferential width
'   * Detects columns with "Experience" or "Description" headers
'   * Makes those columns 30% wider than default
'
' v1.7.17 COMPLETE FIX:
' - CRITICAL: Format ONLY body content, not entire document
'   * Prevents formatting from affecting signature block, headers outside body, etc.
'   * Applies formatting to bodyCC.Range instead of doc.Range
'   * Much cleaner approach - only format what's inside body content control
'
' - Regular paragraphs: 0.5" left indent, NO hanging indent (NOW CHANGED TO 0" IN v1.7.18)
'   * User requirement: "half inch indent, but not hanging indent"
'   * LeftIndent = 0.5", FirstLineIndent = 0"
'
' - Bullets: Proper hanging indent (NOW CHANGED TO 0.5" IN v1.7.18)
'   * LeftIndent = 0.25", FirstLineIndent = -0.25" (hanging)
'   * Bullet at 0", text at 0.25"
'   * Font: Calibri 11pt
'
' - Tables: Roboto font, no indents
'   * Explicit font setting in each cell
'   * LeftIndent = 0", RightIndent = 0"
'
' - Headers: 0" indent
'   * Explicit LeftIndent = 0" in ApplyHeaderStyles
'   * Roboto 16pt
'
' APPROACH:
' 1. ApplyGlobalFormatting formats ONLY body content control (not whole doc)
' 2. FixAllTables overrides table cells with Roboto font and 0" indents
' 3. ConvertTextBulletsToRealBullets applies hanging indent to bullets
' 4. ApplyHeaderStyles sets 0" indent for headers
'
' Result: "Everything has a single baseline" - consistent formatting
'
...
Public  Const BWS_VERSION   As String = "v1.7.28"
...
' -------- Formatting prefs --------
Private Const PARA_LEFT_IN As Double = 0  ' NEW v1.7.18: Zero indent - vertical baseline at left
Private Const BULLET_LEFT_IN As Double = 0.5  ' NEW v1.7.18: Standard half-inch bullet indent
Private Const BULLET_HANG_IN As Double = 0.25  ' NEW v1.7.19: Quarter-inch hanging indent (bullet at 0.25", text at 0.5")
...
' NEW v1.7.20: Table width and column sizing
' NEW v1.7.21: Precise column widths (not multipliers)
' NEW v1.7.22: Table width adjusted to 7.75"
' NEW v1.7.23: Column 3 width adjusted to 1.375"
Private Const TABLE_WIDTH_IN As Double = 7.75  ' inches - preferred table width
Private Const COL1_WIDTH_IN As Double = 1.45  ' inches - Tier column
Private Const COL2_WIDTH_IN As Double = 5     ' inches - Experience Focus / Description column
Private Const COL3_WIDTH_IN As Double = 1.375   ' inches - Budget column (changed from 1.7)
...
' ============================================================================
' AUTOOPEN/AUTONEW - Makes toolbar persistent (NEW v1.7.28: More robust)
' ============================================================================

Public Sub AutoOpen()
    ' NEW v1.7.28: Enhanced logic for Normal.dotm persistence
    ' - Always check if toolbars exist
    ' - Recreate silently if missing
    ' - No user interruption on Word startup
    On Error Resume Next

    ' Check if toolbars exist, rebuild if needed (silent = False means no message)
    If Not BWS_ToolbarsExist() Then
        BWS_RemoveToolbar  ' Clean slate
        BuildOrRefreshBWSToolbar False  ' Rebuild silently
    End If
End Sub

Public Sub AutoNew()
    ' NEW v1.7.28: Same logic as AutoOpen for consistency
    On Error Resume Next
...
Public Sub BWS_Install()
    ...
    ' NEW v1.7.28: SOP - Always uninstall old toolbar first for clean install
    BWS_RemoveToolbar
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.28 Installer!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_Settings()
    ' NEW v1.7.29: Fixed compile error - use late binding to avoid compile-time reference
    ' Try UserForm first, fall back to InputBox
    ' UserForm provides much better UX but requires manual setup
    ' See UserForm setup instructions at end of file

    On Error Resume Next
    Dim frm As Object

    ' Try to create/show the UserForm
    Set frm = VBA.UserForms.Add("BWS_SettingsForm")
    If Not frm Is Nothing Then
        ' Form exists - show it directly (no compile-time reference needed)
        frm.Show
        Set frm = Nothing
    Else
        ' Fall back to InputBox-based settings
        On Error GoTo 0
        BWS_Settings_InputBox
    End If
    On Error GoTo 0
End Sub

' Fallback InputBox-based settings (original v1.7.27 version)
Private Sub BWS_Settings_InputBox()
    ...
    Do While continueLoop
        ' Build settings display in parts to avoid VBA's 25-line continuation limit
        msg = "╔══════════════════════════════════════════════════════╗" & vbCrLf
        msg = msg & "║              BWS SETTINGS MANAGER                    ║" & vbCrLf
        msg = msg & "╚══════════════════════════════════════════════════════╝" & vbCrLf & vbCrLf
        msg = msg & "Current Configuration:" & vbCrLf
        msg = msg & "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" & vbCrLf & vbCrLf
        msg = msg & "1. Base Folder" & vbCrLf
        msg = msg & "   " & TruncatePath(GetSettingStr("BasePath", "(not set)"), 48) & vbCrLf & vbCrLf
        msg = msg & "2. Drafts Folder" & vbCrLf
        msg = msg & "   " & TruncatePath(GetSettingStr("DraftsPath", "(not set)"), 48) & vbCrLf & vbCrLf
        msg = msg & "3. Template File" & vbCrLf
        msg = msg & "   " & TruncatePath(GetSettingStr("TemplatePath", "(not set)"), 48) & vbCrLf & vbCrLf
        msg = msg & "4. Auto-Open Folder:  " & GetSettingStr("AutoOpenFolder", "False") & vbCrLf & vbCrLf
        msg = msg & "5. Bullet Conversion: " & GetSettingStr(BWS_REG_BULLET_CONVERT, "Yes") & vbCrLf & vbCrLf
        msg = msg & "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" & vbCrLf
        msg = msg & "Enter 1-5 to change a setting, or 0 to exit:"

        userInput = InputBox(msg, "BWS Settings", "0")

        ' Trim and validate input
        userInput = Trim$(userInput)

        ' Exit if user cancelled or entered 0
        If LenB(userInput) = 0 Or userInput = "0" Then
            continueLoop = False
            Exit Do
        End If
        ...
        Select Case userInput
            Case "1"  ' Base folder
                newValue = PickFolder("Select your Dropbox base folder")
                If LenB(newValue) > 0 Then
                    SaveSettingStr "BasePath", newValue
                    MsgBox "✓ Base folder updated successfully!" & vbCrLf & vbCrLf & newValue, vbInformation, "BWS Settings"
                End If

            Case "2"  ' Drafts folder
                newValue = PickFolder("Select your Drafts folder")
                If LenB(newValue) > 0 Then
                    SaveSettingStr "DraftsPath", newValue
                    MsgBox "✓ Drafts folder updated successfully!" & vbCrLf & vbCrLf & newValue, vbInformation, "BWS Settings"
                End If

            Case "3"  ' Template file
                newValue = PickFile("Select your letterhead template", "*.dotm; *.dotx")
                If LenB(newValue) > 0 Then
                    SaveSettingStr "TemplatePath", newValue
                    MsgBox "✓ Template file updated successfully!" & vbCrLf & vbCrLf & newValue, vbInformation, "BWS Settings"
                End If

            Case "4"  ' Auto-open folder
                Dim autoMsg As String
                autoMsg = "Auto-open folder when saving documents?" & vbCrLf & vbCrLf
                autoMsg = autoMsg & "Current setting: " & GetSettingStr("AutoOpenFolder", "False") & vbCrLf & vbCrLf
                autoMsg = autoMsg & "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" & vbCrLf
                autoMsg = autoMsg & "Yes = Enable auto-open" & vbCrLf
                autoMsg = autoMsg & "No  = Disable auto-open" & vbCrLf
                autoMsg = autoMsg & "Cancel = Keep current setting"
                choice = MsgBox(autoMsg, vbYesNoCancel + vbQuestion, "Auto-Open Folder")
            ...
            Case "5"  ' Bullet conversion
                ...
                bulletMsg = "Enable automatic bullet conversion during import?" & vbCrLf & vbCrLf
                bulletMsg = bulletMsg & "Current setting: " & GetSettingStr(BWS_REG_BULLET_CONVERT, "Yes") & vbCrLf & vbCrLf
                bulletMsg = bulletMsg & "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" & vbCrLf
                bulletMsg = bulletMsg & "Yes = Enable auto-conversion" & vbCrLf
                bulletMsg = bulletMsg & "No  = Disable auto-conversion" & vbCrLf
                bulletMsg = bulletMsg & "Cancel = Keep current setting"
                choice = MsgBox(bulletMsg, vbYesNoCancel + vbQuestion, "Bullet Conversion")
                If choice = vbYes Then
                    SaveSettingStr BWS_REG_BULLET_CONVERT, "Yes"
            ...
            Case Else
                MsgBox "Invalid choice: " & userInput & vbCrLf & vbCrLf & "Please enter a number from 1-5, or 0 to exit.", vbExclamation, "BWS Settings"
        End Select
    Loop
End Sub

' Helper function for settings display - truncate long paths
Private Function TruncatePath(ByVal path As String, ByVal maxLen As Long) As String
    If Len(path) <= maxLen Then
        TruncatePath = path
    Else
        ...
        TruncatePath = Left$(path, leftLen) & "..." & Right$(path, rightLen)
    End If
End Function
...
Private Function PickFile(ByVal titleText As String, ByVal filterPattern As String, Optional ByVal initialFolder As String = "") As String
    ...
    With fd
        ...
        ' NEW v1.7.24: Set initial directory if provided
        If LenB(initialFolder) > 0 Then
...
Public Sub BWS_ImportPicked()
    ...
    ' NEW v1.7.24: Create new document from template FIRST (like ImportNewest does)
    ' This was the bug - we were trying to import into active doc without template
    templ = GetTemplatePath(True)
    If LenB(templ) = 0 Then
        MsgBox "No template configured.", vbExclamation, "BWS"
        Exit Sub
    ...
    ' Get drafts path to use as initial directory for file picker
    draftsPath = GetDraftsPath(False)
    If LenB(draftsPath) = 0 Then draftsPath = ""

    ' Pick file (opens in drafts folder)
    sourcePath = PickFile("Select draft to import", "*.docx; *.doc", draftsPath)
    If LenB(sourcePath) = 0 Then Exit Sub  ' User cancelled
...
ErrHandler:
    MsgBox "Failed to create document: " & Err.Description, vbCritical, "BWS"
End Sub
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ' NEW v1.7.25: Simplified to only remove imported signatures
    ' The signature wrapper is now protected at template level - perfect!
    ' No need to modify signature images anymore (STEP 2 removed)
    '
    ' Process: REMOVE any "Sincerely" signatures from body content (imported doc)
    ' The template's protected signature wrapper stays untouched
    ...
    ' Find signature wrapper to know where NOT to delete
    Set sigCC = Nothing
    For Each cc In doc.ContentControls
    ...
    ' Remove imported signatures from body content
    ...
    ' NOTE v1.7.25: STEP 2 (image conversion) removed - signature wrapper is protected in template
    ' The signature image formatting is now handled entirely by the template protection
...
Public Sub BWS_FixBullets()
    ...
    For Each para In doc.Paragraphs
        If para.Range.ListFormat.ListType <> wdListNoNumbering Then
            ...
            ' NEW v1.7.17: Proper hanging indent (from v1.7.15)
            ' Paragraph formatting per perfect document (Hall of Fame 2025-11-13)
            ' LeftIndent: 0.25", Hanging: 0.25" (FirstLineIndent: -0.25")
            With para.Range.ParagraphFormat
                .LeftIndent = InchesToPoints(BULLET_LEFT_IN)  ' 0.25"
                .FirstLineIndent = InchesToPoints(-BULLET_HANG_IN)  ' -0.25" (hanging indent)
                .RightIndent = 0  ' Right: 0"
                .SpaceAfter = 6  ' After: 6 pt
                .LineSpacingRule = wdLineSpaceExactly  ' Exactly 13.8 pt
                .LineSpacing = 13.8
                .TabStops.ClearAll
                .TabStops.Add Position:=InchesToPoints(BULLET_TAB_IN)  ' Tab stop at 0.5"
            End With

            ' Font: Calibri, 11 pt
            para.Range.Font.Name = BULLET_FONT
            para.Range.Font.Size = FONT_SIZE_PREF
        End If
    Next para
...
Private Sub FormatTable(ByVal t As Table)
    ...
    ' NEW v1.7.20: Use fixed 7" width (not 100% of window)
    t.Rows.LeftIndent = 0  ' THE critical line!
    t.Rows.Alignment = wdAlignRowLeft
    t.PreferredWidthType = wdPreferredWidthPoints
    t.PreferredWidth = Application.InchesToPoints(TABLE_WIDTH_IN)  ' 7 inches = 504 points
    t.AutoFitBehavior wdAutoFitFixed  ' Fixed width, don't stretch to window
    ...
    ' NEW v1.7.21: Set exact column widths for 3-column tables
    ' NEW v1.7.23: Column 3 adjusted to 1.375"
    ' Column 1: 1.45" (Tier), Column 2: 5" (Experience Focus), Column 3: 1.375" (Budget)
    If t.Columns.Count = 3 Then
        On Error Resume Next
        t.Columns(1).PreferredWidthType = wdPreferredWidthPoints
        t.Columns(1).PreferredWidth = Application.InchesToPoints(COL1_WIDTH_IN)  ' 1.45"

        t.Columns(2).PreferredWidthType = wdPreferredWidthPoints
        t.Columns(2).PreferredWidth = Application.InchesToPoints(COL2_WIDTH_IN)  ' 5"

        t.Columns(3).PreferredWidthType = wdPreferredWidthPoints
        t.Columns(3).PreferredWidth = Application.InchesToPoints(COL3_WIDTH_IN)  ' 1.375"
        On Error GoTo 0
    End If

    ' Column alignment - apply cell-by-cell to avoid table-wide overrides
    ' NEW v1.7.17: Also ensure Roboto font for tables
    Dim c As Long
    Dim r As Long
    ...
    For c = 1 To t.Columns.Count
        ...
        For r = 1 To t.Rows.Count
            ...
            ' NEW v1.7.17: Set font to Roboto (not Times New Roman or Calibri)
...
Private Function FindDescriptionColumn(ByVal t As Table) As Long
    ' NEW v1.7.18: Find the column that should be wider (description/experience column)
    ' Returns column index, or 0 if not found
    Dim c As Long
    Dim headerText As String

    FindDescriptionColumn = 0

    ' Check if table has at least 1 row (header row)
    If t.Rows.Count < 1 Then Exit Function

    On Error Resume Next

    ' Look for columns with "Experience", "Description", "Goals", or similar in header
    For c = 1 To t.Columns.Count
        headerText = Trim$(LCase$(t.Cell(1, c).Range.Text))
        headerText = Replace$(headerText, Chr$(13), "")
        headerText = Replace$(headerText, Chr$(7), "")

        ' Check for common description column headers
        If InStr(1, headerText, "experience", vbTextCompare) > 0 Or _
           InStr(1, headerText, "description", vbTextCompare) > 0 Or _
           InStr(1, headerText, "goal", vbTextCompare) > 0 Or _
           InStr(1, headerText, "detail", vbTextCompare) > 0 Then
            FindDescriptionColumn = c
            Exit Function
        End If
    Next c

    ' If no keyword match, use the middle column (typically has most text)
    ' For 3-column tables, that's column 2
    If t.Columns.Count = 3 Then
        FindDescriptionColumn = 2
    ElseIf t.Columns.Count > 3 Then
        ' For more columns, use column 2 or 3 depending on count
        FindDescriptionColumn = 2
    End If
...
Private Sub ApplyGlobalFormatting(ByVal doc As Document)
    ...
    ' NEW v1.7.17: Apply formatting ONLY to body content, NOT entire document
    ' This prevents formatting from affecting signature block, etc.
    ' Find body content control
    Set bodyCC = FindBodyContentControl(doc, False)
    ...
    ' NEW v1.7.17: Regular paragraphs get 0.5" indent
    With formatRange
        .Font.Name = useFont
    ...
    For Each para In doc.Paragraphs
        ...
        If para.Range.Font.Size >= 14 Or _
           (Len(txt) > 0 And Len(txt) < 100 And txt = UCase$(txt) And para.Range.Font.Bold = True) Then
            ...
            With para.Range.ParagraphFormat
                .LeftIndent = 0                       ' NEW v1.7.17: Headers have 0" indent
...
Public Sub BWS_RemoveToolbar()
    ' NEW v1.7.28: More thorough cleanup
    ' Remove all variants of BWS toolbars (old and new)
    On Error Resume Next
    Application.CommandBars("BWS").Delete      ' Legacy single toolbar
    Application.CommandBars("BWS-1").Delete    ' Row 1
    Application.CommandBars("BWS-2").Delete    ' Row 2
    Application.CommandBars("BWS-3").Delete    ' Row 3
    On Error GoTo 0
End Sub
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    ' NEW v1.7.28: Simplified logic - Always uninstall first (SOP)
    ' This ensures clean slate and prevents duplicates
    ' Skip only if toolbars exist AND this is a silent AutoOpen call
    If BWS_ToolbarsExist() And Not showMessage Then
        ' Toolbars already exist and this is an AutoOpen call, skip recreation
        Exit Sub
    End If

    ' NEW v1.7.28: Always use BWS_RemoveToolbar for consistency
    BWS_RemoveToolbar
    ...
    ' NEW v1.7.26: Updated tooltips to clarify manual vs auto behavior
    ...
    AddBtn cb2, "Fix Tables", "BWS_FixTables", 144, False, MSO_BUTTON_ICON_AND_CAPTION, "Manually format tables (auto during import)"
    AddBtn cb2, "Convert Bullets", "BWS_ConvertTextBullets", 70, True, MSO_BUTTON_ICON_AND_CAPTION, "Manually convert bullets (auto during import)"
    AddBtn cb2, "Fix Bullets", "BWS_FixBullets", 65, False, MSO_BUTTON_ICON_AND_CAPTION, "Manually fix bullet formatting"
    AddBtn cb2, "Toggle Bullets", "BWS_ToggleBullets", 70, False, MSO_BUTTON_ICON_AND_CAPTION, "Enable/disable auto-convert"
    AddBtn cb2, "Apply Format", "BWS_ApplyFormatting", 487, True, MSO_BUTTON_ICON_AND_CAPTION, "Manually apply formatting (auto during import)"
    ...
    If showMessage Then
        MsgBox "BWS v1.7.28 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    ' Part 1: Header and recent versions (v1.7.28, v1.7.27, v1.7.26, v1.7.25)
    msg = "================ BWS v1.7.28 =================" & vbCrLf
    msg = msg & "Version: " & BWS_VERSION & vbCrLf
    msg = msg & "Host:    " & Application.Name & " " & Application.Version & vbCrLf & vbCrLf
    msg = msg & "NEW in v1.7.28:" & vbCrLf
    msg = msg & "• UserForm settings dialog (manual setup)" & vbCrLf
    msg = msg & "• Improved toolbar auto-install (SOP)" & vbCrLf
    msg = msg & "• Robust AutoOpen for Normal.dotm" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.27:" & vbCrLf
    msg = msg & "• BUGFIX: Fixed line continuation limit" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.25:" & vbCrLf
    msg = msg & "• BUGFIX: Removed signature conversion" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.24:" & vbCrLf
    msg = msg & "• BUGFIX: Import Pick creates template doc" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.23:" & vbCrLf
    msg = msg & "• Col 3 (Budget): 1.375"" (was 1.7"")" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.22:" & vbCrLf
    msg = msg & "• Table width: 7.75"" (was 7.68"")" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.21:" & vbCrLf
    msg = msg & "• Col 1 (Tier): 1.45"" fixed" & vbCrLf
    msg = msg & "• Col 2 (Experience): 5"" fixed" & vbCrLf & vbCrLf
    msg = msg & "From v1.7.19:" & vbCrLf
    msg = msg & "• Bullet marker at 0.25""" & vbCrLf

    ' Part 2: Older versions (v1.6.6, v1.6.5)
    msg2 = vbCrLf
    msg2 = msg2 & "From v1.6.6:" & vbCrLf
    msg2 = msg2 & "• Signature: Fixed content preservation" & vbCrLf
    msg2 = msg2 & "• Signature spacing: 2 blank lines" & vbCrLf
    msg2 = msg2 & "• Image behind text (not obscuring)" & vbCrLf
    msg2 = msg2 & "• Paragraphs: Clear hanging indent" & vbCrLf
    msg2 = msg2 & "• Tables: Cell-by-cell alignment" & vbCrLf & vbCrLf
    msg2 = msg2 & "From v1.6.5:" & vbCrLf
    msg2 = msg2 & "• Bullet indent: ListTemplate override" & vbCrLf
    msg2 = msg2 & "• Table order: Fixed alignment override" & vbCrLf
    msg2 = msg2 & "• Config memory in installer" & vbCrLf

    ' Part 3: Core features and footer
    msg3 = vbCrLf
    msg3 = msg3 & "Core Features:" & vbCrLf
    msg3 = msg3 & "• Line spacing: 276 twips (v1.6.1 fix)" & vbCrLf
    msg3 = msg3 & "• Signature block: 0.125"" indent" & vbCrLf
    msg3 = msg3 & "• Calibri font for bullets" & vbCrLf
    msg3 = msg3 & "• Auto page margins (-0.062"" top)" & vbCrLf
    msg3 = msg3 & "• Table headers: #D9D9D9 gray + bold" & vbCrLf
    msg3 = msg3 & "• Metadata formatting (0"" indent)" & vbCrLf
    msg3 = msg3 & "• 3-row persistent toolbar" & vbCrLf
    msg3 = msg3 & "• Smart table formatting" & vbCrLf
    msg3 = msg3 & "=============================================="
...
' ============================================================================
'
'                    BWS USERFORM SETTINGS DIALOG SETUP
'                          (v1.7.28 and later)
'
' ============================================================================
' ============================================================================
...
' OVERVIEW
' --------
' This section provides complete instructions and code for creating a
' professional settings dialog using a VBA UserForm. The UserForm provides
' a much better user experience than the InputBox fallback.
'
' WHY USERFORM?
' -------------
' - Professional Windows dialog appearance
' - Proper controls: textboxes, checkboxes, buttons
...
' PREREQUISITES
' -------------
' - VBA Editor access (Alt+F11 in Word)
' - Basic understanding of VBA Editor
' - Time: ~10 minutes to set up
'
' SETUP STEPS (DO ONCE)
' ----------------------
' After completing these steps, the UserForm will be permanent in your
' Normal.dotm and will be used instead of the InputBox interface.
'
' ============================================================================
...
' 2. In the Project Explorer, find "Normal" (Normal.dotm)
' 3. Right-click on "Normal" or any folder in it
' 4. Select: Insert > UserForm
' 5. A new blank form appears called "UserForm1"
' 6. Click on the form, then press F4 to open Properties Window
...
' ============================================================================
'
' Use the Toolbox (View > Toolbox if not visible) to add these controls.
' For each control, drag it onto the form, then use F4 to set properties.
'
' --- LABELS (use Label from toolbox) ---
'
' Label1:
'    (Name):    lblBasePath
'    Caption:   Base Folder:
...
'    Height:    18
'
' --- TEXTBOXES (use TextBox from toolbox) ---
'
' TextBox1:
'    (Name):    txtBasePath
'    Left:      108
...
'    Height:    20
'
' --- BUTTONS FOR BROWSING (use CommandButton from toolbox) ---
'
' CommandButton1:
'    (Name):    btnBrowseBase
'    Caption:   ...
...
'    Height:    20
'
' --- CHECKBOXES (use CheckBox from toolbox) ---
'
' CheckBox1:
'    (Name):    chkAutoOpen
...
'    Height:    20
'
' --- ACTION BUTTONS (use CommandButton from toolbox) ---
'
' CommandButton4:
'    (Name):    btnSave
'    Caption:   Save
...
'
' ============================================================================
' STEP 3: ADD CODE TO THE USERFORM MODULE
' ============================================================================
'
' 1. Double-click on the UserForm (BWS_SettingsForm) in Project Explorer
' 2. This opens the code window for the form
' 3. Copy and paste ALL the code below into that window
' 4. Replace any existing code in that window
'
' ------- BEGIN USERFORM CODE (copy from here) -------
...
' ------- END USERFORM CODE (copy to here) -------
'
' ============================================================================
' STEP 4: SAVE AND TEST
' ============================================================================
'
' 1. Save the Normal.dotm project (Ctrl+S or File > Save)
' 2. Close VBA Editor
' 3. Back in Word, click the "Settings" button on the BWS toolbar
' 4. You should now see the professional dialog instead of InputBox!
'
' ============================================================================
' TROUBLESHOOTING
' ============================================================================
'
' Issue: Settings button still shows InputBox
' Fix: The UserForm name must be exactly "BWS_SettingsForm"
'      Check the (Name) property in step 1.
'
' Issue: Error when clicking Settings
' Fix: Make sure you pasted the UserForm code into the form's code module,
'      not into BWS_Module. Double-click the form in Project Explorer.
'
' Issue: Browse buttons don't work
' Fix: Make sure the BWS_Module is in Normal.dotm with PickFolder and
'      PickFile functions available.
'
' Issue: Want to delete UserForm and go back to InputBox
' Fix: Right-click BWS_SettingsForm in Project Explorer > Remove
'      The code will automatically fall back to InputBox.
'
' ============================================================================
' BENEFITS OF USERFORM
' ============================================================================
'
' ✓ Professional appearance matching Windows dialogs
' ✓ All settings visible at once - no navigation
' ✓ Integrated folder/file browsers
' ✓ Better keyboard navigation (Tab, Enter, Escape)
' ✓ Checkboxes for boolean settings (much clearer)
' ✓ Save/Cancel buttons for better control
' ✓ No line continuation limits
' ✓ Easier to maintain and enhance
'
' ============================================================================
' END OF USERFORM SETUP INSTRUCTIONS
' ============================================================================
