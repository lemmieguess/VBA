...
' ============================================================================
' Bridgewater Studio - BWS v1.7.20 - Table Width & Column Balance Fix
' Based on: v1.7.19 with table sizing improvements
'
' v1.7.20 USER REQUIREMENTS:
' - "Lost the preferred table width of 7""
'   * Set table width to 7" fixed (not 100% of window)
'   * PreferredWidthType = wdPreferredWidthPoints
'   * PreferredWidth = 504 points (7 * 72)
'
' - "Budget column is off the page and too wide"
'   * Budget column should be tight to numbers
'   * Let currency columns size to content naturally
'
' - "Experience Focus should be 25% wider" (not 30%)
'   * Changed from 1.3x to 1.25x multiplier
'
' - "Tier column width is good"
'   * Keep tier column as-is
'
...
Public  Const BWS_VERSION   As String = "v1.7.20"
...
Private Const TABLE_WIDTH_IN As Double = 7  ' inches - preferred table width
Private Const DESC_COL_MULTIPLIER As Single = 1.25  ' 25% wider for description/experience columns
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.20 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub FormatTable(ByVal t As Table)
    ...
    ' NEW v1.7.18: Make description/experience columns wider
    ' NEW v1.7.20: Changed from 30% to 25% wider
    ' Detect columns with "Description", "Experience", or middle column with lots of text
    Dim descColIndex As Long
    descColIndex = FindDescriptionColumn(t)

    If descColIndex > 0 And t.Columns.Count > 1 Then
        On Error Resume Next
        ' First, AutoFit to content to get natural sizes
        t.AutoFitBehavior wdAutoFitContent

        ' Get current width and make description column 25% bigger
        Dim currentWidth As Single
        currentWidth = t.Columns(descColIndex).Width
        t.Columns(descColIndex).Width = currentWidth * DESC_COL_MULTIPLIER  ' 1.25x = 25% wider
        On Error GoTo 0
    End If
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.7.20 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    ' Part 1: Header and recent versions (v1.7.20, v1.7.19, v1.7.18)
    msg = "================ BWS v1.7.20 =================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.20:" & vbCrLf _
        & "• Table width: Fixed 7"" (not 100%)" & vbCrLf _
        & "• Description column: 25% wider (was 30%)" & vbCrLf _
        & "• Budget column: Tight to numbers" & vbCrLf _
        & vbCrLf _
