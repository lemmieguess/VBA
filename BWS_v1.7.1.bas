...
' ============================================================================
' Bridgewater Studio - BWS v1.7.1 - Import Bugfixes
' Based on: v1.7.0 with three critical import fixes
'
' v1.7.1 BUGFIXES:
' - Fixed picture formatting: IN FRONT of text (msoBringToFront not msoSendToBack)
'   Signature image now visible and positioned correctly
' - Fixed duplicate signatures: Only process signature in body content control
'   Template signature (if exists) is not modified/duplicated
' - Fixed whitespace: Strip empty paragraphs from body content control specifically
'   No more white space at start of imported content
'
' v1.7.0 NEW FEATURE:
...
Public  Const BWS_VERSION   As String = "v1.7.1"
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.1 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ...
    ' ONLY processes signatures INSIDE body content control (not template signature)
    ' Adds spacing after greeting, fixes image wrapping to be in front of text
    ...
    If bodyCC Is Nothing Then
        ' No body content control, process entire document
        bodyStart = 0
        bodyEnd = doc.Range.End
    Else
        ' Only process content inside body content control
        bodyStart = bodyCC.Range.Start
        bodyEnd = bodyCC.Range.End
    End If

    ' Find signature line INSIDE body content control
    For Each para In doc.Paragraphs
        ' Skip paragraphs outside body content control
        If para.Range.Start < bodyStart Or para.Range.Start > bodyEnd Then
            GoTo NextPara
        End If
...
NextPara:
    ...
    If Not sigStart Is Nothing Then
        ...
        lineCount = 0
        For i = sigStart.Range.Start To bodyEnd
            Set para = doc.Range(i, i + 1).Paragraphs(1)
            If para.Range.Start >= sigStart.Range.Start And para.Range.Start <= bodyEnd Then
                ' ONLY fix image wrapping - position IN FRONT of text
                For Each shp In para.Range.InlineShapes
                    If shp.Type = wdInlineShapePicture Or shp.Type = wdInlineShapeLinkedPicture Then
                        ...
                        fltShp.ZOrder msoBringToFront  ' Bring to front so signature is visible
                    End If
                Next shp
...
Private Sub StripBodyContentWhitespace(ByVal doc As Document)
    ...
    ' Find leading empty paragraphs in body content
    For Each para In doc.Paragraphs
        If para.Range.Start >= bodyStart And para.Range.End <= bodyEnd Then
            If Len(Trim$(para.Range.Text)) = 0 Then
    ...
    ' Delete leading empty paragraphs
    For i = 1 To parasToDelete.Count
        On Error Resume Next
        parasToDelete(i).Range.Delete
        On Error GoTo 0
    Next i

    ' Refresh body content range after deletions
    bodyStart = bodyCC.Range.Start
    bodyEnd = bodyCC.Range.End
    Set parasToDelete = New Collection

    ' Find trailing empty paragraphs in body content (reverse)
    For i = doc.Paragraphs.Count To 1 Step -1
        Set para = doc.Paragraphs(i)
        If para.Range.Start >= bodyStart And para.Range.End <= bodyEnd Then
            If Len(Trim$(para.Range.Text)) = 0 Then
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.7.1 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    msg = "================ BWS v1.7.1 ==================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.1:" & vbCrLf _
        & "• Picture: IN FRONT of text (fixed)" & vbCrLf _
        & "• Signatures: No more duplicates" & vbCrLf _
        & "• Whitespace: Stripped from body" & vbCrLf _
        & vbCrLf _
