...
' ============================================================================
' Bridgewater Studio - BWS v1.7.14 - Bullet Formatting Specification
' Based on: v1.7.13 with table formatting fixes
'
' v1.7.14 ENHANCEMENT:
' - Updated bullet formatting to match exact user specification (2025-11-13_01-21-58.png)
'   Added explicit paragraph properties for bullets:
'   * RightIndent = 0" (was not explicitly set)
'   * SpaceAfter = 6 pt (matches body text)
'   * LineSpacing = Exactly 13.8 pt (matches body text)
'   * Tab stop at 0.5" (was missing in ConvertTextBulletsToRealBullets)
'   * Font.Size = 11 pt (explicitly set, not just inherited)
'   This ensures bullets have consistent formatting matching the template specification
'
...
Public Sub BWS_FixBullets()
    ...
    For Each para In doc.Paragraphs
        If para.Range.ListFormat.ListType <> wdListNoNumbering Then
            ...
            ' Override Word's default list formatting to match exact specification
            ' Per user requirements (2025-11-13_01-21-58.png):
            ' - Aligned at: 0"
            ' - Tab after: 0.25"
            ' - Indent at: 0.25"
            With para.Range.ListFormat.ListTemplate.ListLevels(1)
                .NumberPosition = 0  ' Aligned at: 0"
                .TextPosition = InchesToPoints(BULLET_LEFT_IN)  ' Indent at: 0.25"
                .TabPosition = InchesToPoints(BULLET_LEFT_IN)  ' Tab after: 0.25"
                .Alignment = wdListLevelAlignLeft
            End With

            ' Paragraph formatting per user specification
            With para.Range.ParagraphFormat
...
Public Sub BWS_About()
    ...
    msg = "================ BWS v1.7.14 =================" & vbCrLf _
        ...
        & "• Bullet formatting per user spec" & vbCrLf _
        & "• Right indent: 0"" (explicit)" & vbCrLf _
        & "• Space after: 6 pt" & vbCrLf _
        & "• Line spacing: Exactly 13.8 pt" & vbCrLf _
        & "• Tab stop at 0.5"" (consistent)" & vbCrLf _
        & vbCrLf _
        & "From v1.7.13:" & vbCrLf _
        & "• Fixed table indent: RightIndent = 0" & vbCrLf _
        & "• Fixed cell spacing: Before/After = 0" & vbCrLf
