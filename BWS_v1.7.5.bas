...
' ============================================================================
' Bridgewater Studio - BWS v1.7.3 - Simplified Signature and Whitespace
' Based on: v1.7.2 with complete simplification
'
...
Public  Const BWS_VERSION   As String = "v1.7.5"
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ...
    If Not bodyCC Is Nothing Then
                ...
                If LenB(txt) > 0 Then
                    ...
                    If InStr(1, txt, "Sincerely", vbTextCompare) > 0 Or _
                       ...
                       InStr(1, txt, "Regards", vbTextCompare) > 0 Then
                        ' Delete this paragraph and next 5 lines (entire signature block)
        ...
        For i = parasToDelete.Count To 1 Step -1
            ...
            ' Create range spanning this paragraph + next 5 paragraphs (6 total)
            Dim deleteRange As Range
            Set deleteRange = startPara.Range
            Dim paraIndex As Long
            Dim endParaIndex As Long

            paraIndex = startPara.Range.Paragraphs(1).Index
            endParaIndex = paraIndex + 5  ' +5 = 6 total paragraphs to delete

            ' Extend range to cover all 6 paragraphs (if they exist)
            If endParaIndex <= doc.Paragraphs.Count Then
                deleteRange.End = doc.Paragraphs(endParaIndex).Range.End
            Else
                ' Just delete to end of document if less than 6 paragraphs remain
                deleteRange.End = doc.Range.End
            End If

            ' Delete the entire range at once
            deleteRange.Delete
            On Error GoTo 0
        Next i
    End If
...
Public Sub BWS_About()
    ...
    ' Part 1: Header and recent versions (v1.7.5, v1.7.4, v1.7.3)
    msg = "================ BWS v1.7.5 ==================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.5:" & vbCrLf _
        & "• Fixed whitespace detection bug" & vbCrLf _
        & "• Fixed signature deletion loop" & vbCrLf _
        & "• Whitespace stripping now runs LAST" & vbCrLf _
        & vbCrLf _
        & "From v1.7.4:" & vbCrLf _
        & "• Fixed line continuation compiler error" & vbCrLf _
        & vbCrLf _
        & "From v1.7.3:" & vbCrLf _
        & "• DELETE imported signatures" & vbCrLf _
        & "• Only ONE signature (template)" & vbCrLf
