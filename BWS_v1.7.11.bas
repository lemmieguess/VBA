...
' ============================================================================
' Bridgewater Studio - BWS v1.7.11 - Fix Bullet Indent & Signature Issues
' Based on: v1.7.10 with critical formatting fixes
'
' v1.7.11 CRITICAL FIXES:
' - Bullet hanging indent: Proper Word list formatting (bullet at 0.25", text at 0.5")
'   Bullets now have correct hanging indent with wrapped lines aligning properly
' - Signature wrapper duplication: DELETE signature content controls from body content
'   Keeps only ONE signature wrapper (outside body), prevents duplication
' - Signature spacing: Add 2 blank paragraphs between "Sincerely" and name
'   Creates proper visual separation in signature block
' - Image formatting: Ensure "In Front of Text" wrapping in signature
'
...
' -------- Formatting prefs --------
Private Const BULLET_NUMBER_POS As Double = 0.25  ' Where bullet character sits
Private Const BULLET_TEXT_POS As Double = 0.5     ' Where text starts (creates hanging)
Private Const BULLET_LEFT_IN As Double = 0.5      ' Where wrapped lines continue
Private Const BULLET_FIRST_LINE_IN As Double = -0.25  ' Negative = hanging indent
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.11 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub FormatSignatureBlock(ByVal doc As Document)
    ' Three-step process:
    ' 1. DELETE any signature content controls INSIDE body content
    ' 2. REMOVE any "Sincerely" text signatures from body content (imported doc)
    ' 3. Fix image formatting in template signature wrapper (outside body) and add spacing

    Dim cc As ContentControl
    Dim ccToDelete As Collection
    ...
    Dim sincerelyPara As Paragraph
    Dim nextPara As Paragraph

    On Error Resume Next

    ' STEP 0: Find body content control
    Set bodyCC = FindBodyContentControl(doc, False)

    ' STEP 1: Delete any signature content controls INSIDE body content
    ' This prevents duplicate signature wrappers
    If Not bodyCC Is Nothing Then
        Set ccToDelete = New Collection
        ...
        For Each cc In doc.ContentControls
            If InStr(1, cc.Title, "signature", vbTextCompare) > 0 Or _
               ...
               InStr(1, cc.Tag, "SignatureBlock", vbTextCompare) > 0 Then
                ' Check if this content control is INSIDE body content
        ...
        ' Delete signature content controls from body
        For i = ccToDelete.Count To 1 Step -1
            On Error Resume Next
            ccToDelete(i).Delete
            On Error GoTo 0
        Next i
    ...
    ' STEP 1.5: Find the REAL signature wrapper (outside body content)
    Set sigCC = Nothing
    For Each cc In doc.ContentControls
        If InStr(1, cc.Title, "signature", vbTextCompare) > 0 Or _
           InStr(1, cc.Tag, "signature", vbTextCompare) > 0 Or _
           InStr(1, cc.Title, "SignatureBlock", vbTextCompare) > 0 Or _
           InStr(1, cc.Tag, "SignatureBlock", vbTextCompare) > 0 Then
            ' Should be OUTSIDE body content
            If bodyCC Is Nothing Or _
               cc.Range.Start > bodyCC.Range.End Then
                Set sigCC = cc
                Exit For
            End If
        End If
    ...
    ' STEP 2: Remove imported "Sincerely" text signatures from body content
    If Not bodyCC Is Nothing Then
        ...
        For i = parasToDelete.Count To 1 Step -1
            ...
            ' Extend range to cover next 5 paragraphs (6 total including signature lines)
            deleteRange.MoveEnd Unit:=wdParagraph, Count:=5

            ' Ensure we don't delete beyond body content control
    ...
    ' STEP 3: Fix image formatting in template signature wrapper AND add spacing
    If Not sigCC Is Nothing Then
        ' Add spacing between "Sincerely" and name
        ' Find "Sincerely" paragraph in signature wrapper
        For Each para In sigCC.Range.Paragraphs
            txt = Trim$(para.Range.Text)
            If InStr(1, txt, "Sincerely", vbTextCompare) > 0 Then
                ' Add 2 blank paragraphs after "Sincerely"
                Set nextPara = para.Next
                If Not nextPara Is Nothing Then
                    ' Insert blank paragraph before the name
        ...
        For Each shp In sigCC.Range.InlineShapes
            If shp.Type = wdInlineShapePicture Or shp.Type = wdInlineShapeLinkedPicture Then
                ...
                fltShp.WrapFormat.Type = wdWrapFront  ' In front of text
...
Public Sub BWS_FixBullets()
    ...
    For Each para In doc.Paragraphs
        If para.Range.ListFormat.ListType <> wdListNoNumbering Then
            ...
            ' Apply proper bullet hanging indent formatting
            ' Bullet at 0.25", text starts at 0.5", wrapped lines continue at 0.5"
            With para.Range.ListFormat.ListTemplate.ListLevels(1)
                .NumberPosition = InchesToPoints(BULLET_NUMBER_POS)  ' Bullet at 0.25"
                .TextPosition = InchesToPoints(BULLET_TEXT_POS)      ' Text starts at 0.5"
                .TabPosition = InchesToPoints(BULLET_TAB_IN)         ' Tab at 0.5"
                .Alignment = wdListLevelAlignLeft
            End With

            With para.Range.ParagraphFormat
                .LeftIndent = InchesToPoints(BULLET_LEFT_IN)              ' 0.5" for wrapped lines
                .FirstLineIndent = InchesToPoints(BULLET_FIRST_LINE_IN)   ' -0.25" for hanging
            End With
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.7.11 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    msg = "================ BWS v1.7.11 =================" & vbCrLf _
        ...
        & "• Bullet indent: Proper hanging (0.25""/0.5"")" & vbCrLf _
        & "• Signature: Remove duplicates from body" & vbCrLf _
        & "• Signature: Add spacing before name" & vbCrLf _
        & "• Image: In Front of Text wrapping" & vbCrLf _
        & vbCrLf _
        & "From v1.7.10:" & vbCrLf _
        & "• Fixed Dim inside loop (compiler)" & vbCrLf
