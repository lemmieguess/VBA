...
' ============================================================================
' Bridgewater Studio - BWS v1.7.16 - Paragraph Formatting Fix
' Based on: v1.7.15 with paragraph indent corrections
'
' v1.7.16 CRITICAL FIX:
' - Fixed regular paragraph formatting (not bullets)
'   Problem: v1.7.15 applied bullet formatting too broadly, affecting tables/headers
'            Also set LeftIndent=0" for all paragraphs, but user wants 0.5" for regular text
'   Solution: Separate bullet formatting from regular paragraph formatting
'   * Regular paragraphs: Roboto, LeftIndent = 0.5", FirstLineIndent = 0" (NO hanging)
'   * Bullets: Calibri, LeftIndent = 0.25", FirstLineIndent = -0.25" (hanging)
'   * Tables: Roboto font, no indents (explicit LeftIndent = 0)
'   * Headers: LeftIndent = 0" (no indent)
'   * "Everything should have a single baseline" - consistent formatting throughout
'
' v1.7.15 CRITICAL FIX:
' - Fixed bullet hanging indent calculation
'   Problem: v1.7.14 set FirstLineIndent = 0, causing bullets to not hang properly
'   Analysis: Compared "High School Hall of Fame Interactive Wall_2025-11-13.docx" (perfect)
'             vs macro-generated version - found indentation mismatch
'   Solution: Changed FirstLineIndent = 0 to FirstLineIndent = -0.25"
'   * LeftIndent = 0.25" (paragraph indent)
'   * FirstLineIndent = -0.25" (hanging indent for bullet)
'   * Result: Bullet at 0", text at 0.25" (matches perfect document)
'   * Removed ListTemplate manipulation - let ApplyBulletDefault handle it
'
' v1.7.14 ENHANCEMENT:
' - Updated bullet formatting to match user specification
'   Added explicit paragraph properties for bullets
'
...
Public  Const BWS_VERSION   As String = "v1.7.16"
...
' -------- Formatting prefs --------
Private Const PARA_LEFT_IN As Double = 0.5  ' NEW v1.7.16: Regular paragraph indent
...
Public Sub BWS_Install()
    ...
    If Not useExisting Then
        MsgBox "Welcome to BWS v1.7.16 Installer!" & vbCrLf & vbCrLf & _
...
Private Sub FormatTable(ByVal t As Table)
    ...
    ' NEW v1.7.16: Also ensure Roboto font for tables
    Dim c As Long
    Dim r As Long
    ...
    For c = 1 To t.Columns.Count
        ...
        For r = 1 To t.Rows.Count
            ...
            ' NEW v1.7.16: Set font to Roboto (not Times New Roman or Calibri)
...
Private Sub ApplyGlobalFormatting(ByVal doc As Document)
    ...
    ' NEW v1.7.16: Regular paragraphs get 0.5" left indent (bullets/tables/headers overridden later)
    With doc.Range
        .Font.Name = useFont
...
Private Sub BuildOrRefreshBWSToolbar(ByVal showMessage As Boolean)
    ...
    If showMessage Then
        MsgBox "BWS v1.7.16 toolbar installed!" & vbCrLf & vbCrLf & _
...
Public Sub BWS_About()
    ...
    ' Part 1: Header and recent versions (v1.7.16, v1.7.15, v1.7.14)
    msg = "================ BWS v1.7.16 =================" & vbCrLf _
        & "Version: " & BWS_VERSION & vbCrLf _
        & "Host:    " & Application.Name & " " & Application.Version & vbCrLf _
        & vbCrLf _
        & "NEW in v1.7.16:" & vbCrLf _
        & "• Regular paragraphs: 0.5"" indent" & vbCrLf _
        & "• Tables: Roboto font, 0"" indent" & vbCrLf _
        & "• Headers: 0"" indent" & vbCrLf _
        & "• Bullets: Keep Calibri, 0.25"" indent" & vbCrLf _
        & vbCrLf _
        & "From v1.7.15:" & vbCrLf _
        & "• Fixed bullet hanging indent" & vbCrLf _
        & "• FirstLineIndent: -0.25"" (not 0)" & vbCrLf _
        & vbCrLf _
        & "From v1.7.13:" & vbCrLf _
        & "• Fixed table indent: RightIndent = 0" & vbCrLf
